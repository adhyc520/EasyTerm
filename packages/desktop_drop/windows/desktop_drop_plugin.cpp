...
namespace {
    ...
    HRESULT DesktopDropTarget::DragEnter(IDataObject *pDataObj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect) {
        ...
        if (pDataObj != nullptr && pDataObj->QueryGetData(&fmtetc) == S_OK) {
    ...
    HRESULT DesktopDropTarget::Drop(IDataObject *pDataObj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect) {
                ...
                if (pDataObj->GetData(&fmtetc, &stgmed) == S_OK) {
                    ...
                    if (data != nullptr) {
                        ...
                        for (unsigned int i = 0; i < files; ++i) {
                            ...
                            // cch is buffer size in characters (not bytes) for DragQueryFileW
                            DragQueryFile(reinterpret_cast<HDROP>(data), i, filename, MAX_PATH);
        ...
        // OLE often calls DragLeave immediately before Drop(), which clears the
        // last hover position on the Dart side. Re-send coordinates so
        // DropTarget receives a correct DropDoneEvent location (fixes Windows
        // drops being ignored while macOS/Linux still work).
        POINT dropPoint = {pt.x, pt.y};
        ScreenToClient(window_handle_, &dropPoint);
        channel_->InvokeMethod("updated", std::make_unique<flutter::EncodableValue>(
                flutter::EncodableList{
                        flutter::EncodableValue(double(dropPoint.x)),
                        flutter::EncodableValue(double(dropPoint.y))
                }
        ));
        ...
        if (pdwEffect != nullptr) {
            *pdwEffect = list.empty() ? DROPEFFECT_NONE : DROPEFFECT_COPY;
        }
        return 0;
    }
